...
{{{ read("FetchWithProgress.js") }}}
  ...
  function downloadBinaryMerge(url) {
    if (Array.isArray(url)) {
      return Promise.all(url.map(u => downloadBinaryOrig(u, false))).then(function (arrays) {
        ...
        return decompress(arr, url);
      })
    } else {
      return downloadBinaryOrig(url, true)
    }
  }

  function downloadBinary(urlId) {
    return downloadBinaryMerge(Module[urlId])
  }

  function downloadBinaryOrig(url, allowDecompress) {
      progressUpdate(url);
#if USE_DATA_CACHING
...
#if DECOMPRESSION_FALLBACK
        if (allowDecompress) {
          return decompress(response.parsedBody, url);
        } else {
